...
switch _mode do {
    ...
    case "Preload": {
        ...
        private _hm = createLocation ["Invisible", [0,0,0], 0, 0];
        missionNamespace setVariable ["jna_itemTypeHashmap", _hm];


        private _data = EMPTY_ARRAY;
        INITTYPES;
    ...
    case "UpdateItemGui":{
        ...
        private _dataStr = if _type then{_ctrlList lnbData [_l,0]}else{_ctrlList lbdata _l};
        private _data = parseSimpleArray _dataStr;
        ...
        private _ammo_logo = getText(configfile >> "RscDisplayArsenal" >> "Controls" >> "TabCargoMag" >> "text");
        if _type then{
    ...
    case "SelectItemRight": {
        ...
        private _columns = lnbsize _ctrlList select 1;
        ...
        for "_r" from 0 to (_rows - 1) do {
            private _dataStr = _ctrlList lnbData [_r,0];
            private _data = parseSimpleArray _dataStr;
			private _item = _data select 0;
            private _amount = _data select 1;
            ...
            private _grayout = ((_amount <= _min) && (_amount != -1) && (_amount !=0) && !([player] call isMember));

            private _isIncompatible = _ctrlList lnbvalue [_r,1];
            private _mass = _ctrlList lbvalue (_r * _columns);
            private _alpha = [1.0,0.25] select (_mass > parseNumber (str _load));
